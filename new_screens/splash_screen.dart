...
class _SplashScreenState extends State<SplashScreen>
  ...
  late final Animation<double> _lineW;   // horizontal rule grow
  ...
  void initState() {
    ...
    _logoFade = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _logoCtrl, curve: const Interval(0.0, 0.55, curve: Curves.easeOut)));
    _logoY = Tween(begin: 14.0, end: 0.0).animate(
      CurvedAnimation(parent: _logoCtrl, curve: const Interval(0.0, 0.55, curve: Curves.easeOutCubic)));

    _subFade = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _logoCtrl, curve: const Interval(0.4, 0.8, curve: Curves.easeOut)));
    _subY = Tween(begin: 10.0, end: 0.0).animate(
      CurvedAnimation(parent: _logoCtrl, curve: const Interval(0.4, 0.8, curve: Curves.easeOutCubic)));

    _dotFade = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _logoCtrl, curve: const Interval(0.7, 1.0, curve: Curves.easeOut)));
...
class _PulseDotState extends State<_PulseDot> with SingleTickerProviderStateMixin {
  ...
  late final Animation<double> _s = Tween(begin: 0.6, end: 1.0)
      ...
      builder: (_, __) => Transform.scale(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            color: AppColors.gold.withOpacity(0.7 * _s.value),
          ),
        ),
      ),
    );
...
class _SplashBgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const margin = 28.0;
    canvas.drawLine(const Offset(margin, margin), const Offset(margin + cornerLen, margin), linePaint);
    canvas.drawLine(const Offset(margin, margin), const Offset(margin, margin + cornerLen), linePaint);

    // Bottom-right corner bracket
    canvas.drawLine(Offset(size.width - margin, size.height - margin),
        Offset(size.width - margin - cornerLen, size.height - margin), linePaint);
    canvas.drawLine(Offset(size.width - margin, size.height - margin),
        Offset(size.width - margin, size.height - margin - cornerLen), linePaint);
  }
